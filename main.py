...
# Loss & utils fuction
from loss.hmd_loss import HierarchicalMutualDistillationLoss
...
def set_model(opt, device):
    ...
    if opt.hmd_loss:
        loss = HierarchicalMutualDistillationLoss(dataset=opt.dataset, base_temp=opt.temp, base_lambda=opt.lambda_param, alpha=opt.alpha)
